...
static const char *virtualcam_name(void *unused)
{
	...
	return "PipeWire Virtual Camera Output";
}
...
static void virtual_video(void *data, struct video_data *frame)
{
	...
	// copy data
	for (uint i = 0; i < obs_pwvc->formatinfo.planes; i++) {
		if (d[0].data == NULL) {
		...
		d[0].mapoffset = 0;
		d[0].maxsize = obs_pwvc->formatinfo.sizes[i];
		d[0].flags = SPA_DATA_FLAG_READABLE;
		d[0].type = SPA_DATA_MemPtr;
		d[0].chunk->offset = 0;
		d[0].chunk->stride = obs_pwvc->formatinfo.strides[i];
		d[0].chunk->size = obs_pwvc->formatinfo.sizes[i];
	}
